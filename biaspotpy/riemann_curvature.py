...
class ReactionPathRicciCurvature:
    ...
    def calc_Riemann_2nd_derivative_metric(self, i, j, a, b):
        ...
        if (b == i and b == j) or (a == i and a == j):
            ...
            d2g_dt2_list = [((self.jacob_mat[2][n][a] - self.jacob_mat[0][n][a]) * self.jacob_mat[1][n][i]) / ((self.geod_mat[2][a] - self.geod_mat[1][a]) * (self.geod_mat[1][a] - self.geod_mat[0][a])) for n in range(len(self.jacob_mat[1]))]
            d2g_dt2 = sum(d2g_dt2_list)
        else:
            d2g_dt2_list = [(((self.jacob_mat[2][n][b] - self.jacob_mat[1][n][b]) * self.jacob_mat[1][n][a]) - ((self.jacob_mat[2][n][b] - self.jacob_mat[1][n][b]) * self.jacob_mat[0][n][a]))/ ((self.geod_mat[2][a] - self.geod_mat[1][a]) * (self.geod_mat[1][a] - self.geod_mat[0][a])) for n in range(len(self.jacob_mat[1]))]
    ...
    def calc_Riemann_inv_metric(self, k, i, j):
        g_inv_list = [(1/self.jacob_mat[k][n][i]) * (1/self.jacob_mat[k][n][j]) for n in range(len(self.jacob_mat[k]))]

        return sum(g_inv_list)
...
class CalculationCurvature:
    ...
    def main(self):
        ...
        for i in range(2, step_num - 3):
            ...
            ricci_curvature = RPRC.calc_Ricci_curvature()#TODO: Too much elapse time of calulation is required.
        ...
        G.single_plot(num_list, rxn_path_ricci_curvature_list, "", 0, axis_name_1="ITR. ", axis_name_2="Ricci_curvature", name="Ricci_curvature")
        with open(self.BPA_FOLDER_DIRECTORY+"ricci_curvature.csv", "w") as f:
            f.write("iter., Ricci curvature\n")
